...
class _RasterizeMaskGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, mask, radii, geomBuffer, binningBuffer, imgBuffer = (
                    _C.rasterize_mask_gaussians(*args)
                )
            except Exception as ex:
                torch.save(cpu_args, "snapshot_fw.dump")
                print(
    ...
    def backward(ctx, grad_out_mask, _):
        ...
        grads = (
            ...
            grad_mask,
            torch.zeros_like(
                scales, dtype=torch.float32, device=grad_mask.device, requires_grad=True
            ),
            torch.zeros_like(
                rotations,
                dtype=torch.float32,
