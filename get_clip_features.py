...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        for i, image_path in tqdm(
            enumerate(sorted(os.listdir(os.path.join(args.image_root, "images"))))
        ):
            # print(image_path)
            image = cv2.imread(
                os.path.join(os.path.join(args.image_root, "images"), image_path)
            )
            image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
            masks = torch.load(
                os.path.join(
                    os.path.join(args.image_root, "sam_masks"),
            ...
            # N_mask, C
            features = get_features_from_image_and_masks(
                clip_model, image, masks, background=0.0
            )
