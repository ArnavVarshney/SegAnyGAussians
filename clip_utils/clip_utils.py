...
class OpenCLIPNetworkConfig:
    ...
    clip_model_pretrained: str = (
        "/userhome/30/varshney/CLIP-ViT-B-16-laion2B-s34B-b88K/open_clip_pytorch_model.bin"
    )
    clip_n_dims: int = 512
    negatives: Tuple[str] = (
        "object",
...
class OpenCLIPNetwork(nn.Module):
    ...
    def get_relevancy_with_template(self, embed: torch.Tensor) -> torch.Tensor:
        ...
        repeated_pos = positive_vals.repeat(
            1, 1, len(self.negatives)
        )  # rays x N_pos x N_neg
        repeated_neg = negative_vals.repeat(
            1, len(self.pos_embeds), 1
        )  # rays x N_pos x N_neg
        sims = torch.stack(
            (repeated_pos, repeated_neg), dim=-1
        )  # rays x N_pos x N_neg x 2
        softmax = torch.softmax(
            10 * sims, dim=-1
        )  # N_image_features x N_pos x N_neg x 2
