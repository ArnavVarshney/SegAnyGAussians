...
@torch.no_grad()
def get_features_from_image_and_masks(
    clip_model: OpenCLIPNetwork, image: np.array, masks: torch.tensor, background=1.0
):
    ...
    masks = torch.nn.functional.interpolate(
        masks.unsqueeze(0).float(), image_shape, mode="bilinear"
    ).squeeze(0)
    ...
    masked_images = (
        masks[:, :, :, None] * original_image
        + (1 - masks[:, :, :, None]) * 255.0 * background
    )
    ...
    for seg_idx in range(len(bboxes)):
        with torch.no_grad():
            tmp_image = masked_images[seg_idx][
                bboxes[seg_idx][1] : bboxes[seg_idx][3],
                bboxes[seg_idx][0] : bboxes[seg_idx][2],
                :,
            ]
...
def get_scores_with_template(
    clip_model, images_features, prompt, template=default_template
):
    with torch.no_grad():
        clip_model.set_positives([t.format(prompt) for t in template])
        r_scores = []
...
def get_segmentation(clip_model, images_features, images_masks, prompts=[]):
    ...
    for i, f in tqdm(enumerate(images_features)):
        with torch.no_grad():
            ...
            # n_p, h, w
            image_score = torch.einsum(
                "kp,khw->kphw", segmentation_score, images_masks[i]
            ).sum(dim=0) / (images_masks[i].sum(dim=0, keepdim=True) + 1e-9)
        images_scores.append(image_score)
    return images_scores
...
def get_3d_mask(
    ...
    for it, view in tqdm(enumerate(cameras)):
        ...
        rendered_mask_pkg = gaussian_renderer.render_mask(
            view,
            scene_gaussians,
            pipeline.extract(args),
            background,
            precomputed_mask=tmp_mask,
        )

        gt_score = images_scores[image_idx]

        tmp_target_mask = torch.nn.functional.interpolate(
            gt_score.unsqueeze(0).unsqueeze(0).float(),
            size=rendered_mask_pkg["mask"].shape[-2:],
            mode="bilinear",
        ).squeeze(0)
...
def load_multi_lvl_features_and_masks(
    image_path="./data/3dovs/bed/images/",
    feature_path="./data/3dovs/bed/language_features/",
):
    ...
    for image_name in sorted(os.listdir(image_path)):
        ...
        for lvl, mask in enumerate(tmp_s):
            ...
            images_features[lvl].append(
                tmp_f[all_features : all_features + num_masks_in_lvl]
            )
...
def get_multi_lvl_scores(clip_model, images_features, images_masks, prompt):
    ...
    for lvl, fs_in_lvl in tqdm(enumerate(images_features)):
        ...
        for i, f in enumerate(fs_in_lvl):
            ...
            with torch.no_grad():
                relevancy_score = clip_model.get_relevancy(
                    torch.from_numpy(f).cuda(), 0
                )
                r_score = relevancy_score[:, 0]

                stacked_image_mask = torch.from_numpy(
                    np.stack(images_masks[lvl][i])
                ).cuda()
    ...
    for i in range(len(images_scores[1])):
        final_images_scores.append(
            torch.stack(
                [images_scores[1][i], images_scores[2][i], images_scores[3][i]], dim=0
            )
        )
