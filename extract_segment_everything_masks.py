    ...
    for path in process_iterator:
        ...
        if args.downsample_manually:
            img = cv2.resize(
                img,
                dsize=(
                    img.shape[1] // args.downsample,
                    img.shape[0] // args.downsample,
                ),
                fx=1,
                fy=1,
        ...
        if mask_list:
            ...
            torch.save(masks.cpu(), output_file)
        else:
            torch.save(
                torch.zeros((0, img.shape[0], img.shape[1]), dtype=torch.bool),
                output_file,
            )
...
if __name__ == "__main__":
    parser = ArgumentParser(
        description="SAM segment everything masks extracting params"
    )
