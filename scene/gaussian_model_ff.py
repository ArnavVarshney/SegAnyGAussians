...
class FeatureGaussianModel:
    ...
    def change_to_segmentation_mode(
        self, training_args=None, target="coarse_seg_everything", fixed_feature=False
    ):

        if target == "coarse_seg_everything":
            self._point_features.data[:, :] = 0
        elif target == "contrastive_feature":
            self._point_features.data = (
                torch.randn_like(self._point_features.data) * 1e-2
            )
    ...
    def change_to_multi_res_feature_mode(self, training_args=None):
        assert (
            self.idx_mapper is not None
            and self.multi_res_features is not None
            and "Multi res feature mode not initialized!"
        )

        # self._point_features = torch.empty(0)
        self._point_features.data = torch.zeros(
            (self._point_features.shape[0], 2), device=self._point_features.data.device
        )
        ...
        if training_args is not None:
            ...
            l = [
                # {'params': [self._xyz], 'lr': training_args.position_lr_init * self.spatial_lr_scale, "name": "xyz"},
                {
                    "params": [self.multi_res_features[0]],
                    "lr": training_args.feature_lr,
                    "name": "f",
                },
                {
                    "params": [self.multi_res_features[1]],
                    "lr": training_args.feature_lr,
                    "name": "f",
                },
                {
                    "params": [self.multi_res_features[2]],
                    "lr": training_args.feature_lr,
                    "name": "f",
                },
                {
    ...
    def initialize_multi_res_feature(self, path=None):
        ...
        self.idx_mapper = torch.zeros(
            (self._point_features.shape[0], 3), dtype=torch.long, device="cuda"
        )
        fds = [10, 10, 10]
        ...
        for l, stride in enumerate([0.05, 0.13, 0.23]):
            ...
            idx_xyz = (
                norm_xyz.ceil()[:, 0].long()
                + norm_xyz.ceil()[:, 1].long() * num_x
                + norm_xyz.ceil()[:, 2].long() * num_x * num_y
            )
            ...
            self.multi_res_features.append(
                nn.Parameter(
                    torch.zeros((len(uni), fds[l]), dtype=torch.float, device="cuda")
                    .contiguous()
                    .requires_grad_(True)
                )
            )
    ...
    def load_multi_res_idx_mapper(self, path=None):
        ...
        for i in range(3):
            ...
            self.multi_res_features.append(
                nn.Parameter(
                    torch.zeros((len(uni), fds[i]), dtype=torch.float, device="cuda")
                    .contiguous()
                    .requires_grad_(True)
                )
            )
    ...
    def smooth_point_features(self, K=16, smoothed_dim=24):
        ...
        cur_features[:, :smoothed_dim] = (
            cur_features[self.feature_smooth_map["m"], :smoothed_dim]
            .mean(dim=1)
            .detach()
        )
    ...
    def get_smoothed_point_features(self, K=16, dropout=0.5):
        ...
        normed_features = torch.nn.functional.normalize(
            self._point_features, dim=-1, p=2
        )
    ...
    def get_multi_resolution_smoothed_point_features(
        self, sample_rates=(0.1, 0.5, 1.5), Ks=(4, 4, 16), smooth_weights=None
    ):
        assert len(sample_rates) == len(Ks) and (
            smooth_weights is None or smooth_weights.shape[1] == len(Ks)
        )
        ...
        with torch.no_grad():
            ...
            for i, (r, k) in enumerate(zip(sample_rates, Ks)):
                if (
                    len(self.multi_res_feature_smooth_map) <= i
                    or self.multi_res_feature_smooth_map[i]["rate"] != r
                    or self.multi_res_feature_smooth_map[i]["K"] != k
                ):
                    ...
                    if len(self.multi_res_feature_smooth_map) <= i:
                        self.multi_res_feature_smooth_map.append(
                            {"rate": r, "K": k, "point_mask": pm, "m": nearest_k_idx}
                        )
                    else:
                        self.multi_res_feature_smooth_map[i] = {
                            "rate": r,
                            "K": k,
                            "point_mask": pm,
                            "m": nearest_k_idx,
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        # features[:, 3:, 1:] = 0.0
        features = (
            torch.zeros((fused_point_cloud.shape[0], self.feature_dim)).float().cuda()
        )
        ...
        dist2 = torch.clamp_min(
            distCUDA2(torch.from_numpy(np.asarray(np_pcd_points)).float().cuda()),
            0.0000001,
        )
    ...
    def save_ply(self, path, smooth_weights=None, smooth_type=None, smooth_K=None):
        ...
        if smooth_weights is not None:
            f = (
                self.get_multi_resolution_smoothed_point_features(
            ...
            f = (
                self.get_smoothed_point_features(K=smooth_K, dropout=-1)
                .detach()
                .contiguous()
                .cpu()
                .numpy()
        ...
        attributes = np.concatenate(
            (xyz, normals, f, opacities, scale, rotation), axis=1
        )
    ...
    def load_ply(self, path):
        ...
        f_names = [
            p.name for p in plydata.elements[0].properties if p.name.startswith("f_")
        ]
        ...
        self._point_features = nn.Parameter(
    ...
    def densification_postfix(
        self, new_xyz, new_point_features, new_opacities, new_scaling, new_rotation
    ):
        d = {
            "xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        self.densification_postfix(
            new_xyz, new_point_features, new_opacity, new_scaling, new_rotation
        )
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        self.densification_postfix(
            new_xyz, new_point_features, new_opacities, new_scaling, new_rotation
        )
